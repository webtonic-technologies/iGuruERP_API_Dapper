...
namespace FeesManagement_API.Repository.Implementations
{
    public class NumberSchemeRepository : INumberSchemeRepository
    {
        ...
        public async Task<int> CountActiveNumberSchemes(int instituteID)
        {
            var query = @"SELECT COUNT(*) 
                      FROM tblNumberScheme 
                      WHERE InstituteID = @InstituteID AND IsActive = 1";
            return await _connection.ExecuteScalarAsync<int>(query, new { InstituteID = instituteID });
        }
        ...
        FROM tblNumberScheme ns
        LEFT JOIN tblSchemeType st ON ns.SchemeTypeID = st.SchemeTypeID
        WHERE ns.NumberSchemeID = @NumberSchemeID AND ns.IsActive = 1";
        ...
        {
            var query = @"SELECT SchemeTypeID, SchemeType FROM tblSchemeType";
            var schemeTypes = await _connection.QueryAsync<NumberSchemeTypeResponse>(query);
            return schemeTypes;
        }
    }
}

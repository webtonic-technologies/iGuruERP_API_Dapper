        ...
        public List<StudentFeeResponse> GetStudentFees(StudentFeeRequest request)
        {
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                var query = @"
                         WITH UniqueLFRS AS (
    SELECT DISTINCT 
           LateFeeRuleID, FeeHeadID, FeeTenurityID, DueDate, InstituteID, IsActive
...
PaymentFromTransaction AS (
    SELECT 
       p.StudentID, p.ClassID, p.SectionID, p.InstituteID,
...
SELECT  
    ...
    -- override base fee by any student‐specific discount record
    COALESCE(
...
LEFT JOIN tblStudentDiscount sd
   ...
   -- match the same TenuritySTMID / FeeCollectionSTMID
...
";
        ...
        //            foreach (var req in requests)
        //            {
        //                var id = connection.QuerySingle<int>(sql, req, transaction: tx);
        //                ids.Add(id);
        //            }
        ...
        public async Task<IEnumerable<StudentFeeRawData>> GetStudentFeeRawDataAsync(GetStudentFeesExportRequest request)
        {
            string sql = @"
            WITH UniqueLFRS AS (
                SELECT DISTINCT 
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var result = await connection.QueryAsync<StudentFeeRawData>(sql, parameters);
                return result;
            }
        }


        public async Task<IEnumerable<GetFeesChangeLogsExportResponse>> GetFeesChangeLogsExportAsync(GetFeesChangeLogsExportRequest request)
        {
            string sql = @"
                SELECT
                    sm.Admission_Number AS AdmissionNumber,
            ...
            using (var connection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                var result = await connection.QueryAsync<GetFeesChangeLogsExportResponse>(sql, parameters);
                return result;
            }
        }
